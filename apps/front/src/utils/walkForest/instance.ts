...
> {
  ...
  private eachRootBefore(fn: ForestCb<T, VK>) {
    ...
    while (forestQueue.length > 0) {
      ...
      while (treeNode) {
        ...
        const nodePath = [...forestWithPath.nodePath, treeNode];

        if (fn(treeNode, path, nodePath)) return;
  ...
  private eachRootAfter(fn: ForestCb<T, VK>) {
    ...
    const addTreeToQueue = (
      forest: T[],
      parentPath: Array<T[VK]>,
      parentNodePath: Array<T>
    ) => {
      ...
      while (treeNode) {
        nodeStack.push({
          ...
          nodePath: [...parentNodePath, treeNode],
        });
        treeNode = next();
      }
    };
    ...
    while (nodeStack.length > 0) {
      ...
      if (!lastNode.isWalkChildren && lastNode.node[this.subTreeKey]) {
        ...
        addTreeToQueue(
          lastNode.node[this.subTreeKey],
          lastNode.path,
          lastNode.nodePath
        );
      } else {
        if (fn(lastNode.node, lastNode.path, lastNode.nodePath)) {
          return;
        }
      }
    }
  ...
  filterWithPath(fn: ForestCb<T, VK>) {
    ...
    this.each((item, path, nodePath) => {
      const isMatch = fn(item, path, nodePath);
